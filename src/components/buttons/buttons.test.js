import { ActionButton, ToggleActionButton } from ".";
import { shallow,  mount} from 'enzyme';
import React from 'react';
import Theme from '../../../.storybook/theme';
import 'jest-styled-components';

import { ThemeProvider } from "styled-components";


describe ('ActionButton', () => {
  it('When disabled, button is unclickable & color changed', () => {
    let value = 0;

    // test if value adds up to 1 when clicked
   const wrapperEnabled = shallow(<ActionButton onClick={() => value++}>Testje1</ActionButton>)
   wrapperEnabled.simulate('click');
   expect(value).toEqual(1);

   // test if value stays 1 when button is disabled and clicked
   const wrapperDisabled = shallow(<ActionButton disabled onClick={() => value++}>Testje2</ActionButton>)
   wrapperDisabled.simulate('click');
   expect(value).toEqual(1);

   // test if style is correct when disabled
   const wrapper = mount(
    <ThemeProvider theme={Theme}>
      <ActionButton disabled onClick={() => value++}>Testje2</ActionButton>
    </ThemeProvider>
   )
   expect(wrapper.childAt(0)).toHaveStyleRule('background-color', Theme().colors.disabled);
  });

  it('can pass extra props', () => {
   const wrapper = shallow(<ActionButton otherProps={{id: '31'}} onClick={() => console.log('testen')}>Testje</ActionButton>)
   expect(wrapper.props().id).toEqual('31');
  })

  it('style changes when level changes', () => {

    const wrapper = mount(
      <ThemeProvider theme={Theme}>
        <ActionButton onClick={() => value++}>Testje2</ActionButton>
      </ThemeProvider>
     );

     // test no level
     expect(wrapper.childAt(0)).toHaveStyleRule('background-color', Theme().colors.action)
     
     // test primary level
     wrapper.setProps({ children: <ActionButton level='primary' onClick={() => value++}>Testje2</ActionButton>});
     expect(wrapper.childAt(0)).toHaveStyleRule('background-color', Theme().colors.action);

     // test secondary level
     wrapper.setProps({ children: <ActionButton level='secondary' onClick={() => value++}>Testje2</ActionButton>});
     expect(wrapper.childAt(0)).toHaveStyleRule('background-color', Theme().colors.interactive);

     // test random level
     wrapper.setProps({ children: <ActionButton level='randomword' onClick={() => value++}>Testje2</ActionButton>});
     expect(wrapper.childAt(0)).toHaveStyleRule('background-color', Theme().colors.interactive);
  })

  it('fullwidth prop takes 100% of width', () => {
    const wrapper = mount(<ActionButton fullwidth onClick={() => console.log('testen')}>Testje</ActionButton>)
    expect(wrapper).toHaveStyleRule('width', '100%');
  });
  
  it('icon display', () => {
    // test with image string
    const wrapper = mount(<ActionButton icon='data:image/png;base64,' onClick={() => console.log('testen')}>Testje</ActionButton>)
    expect(wrapper.exists('img')).toEqual(true);
    
    //empty icon tag
    wrapper.setProps({icon: ''});
    expect(wrapper.exists('img')).toEqual(false);
  });
});

describe ('ToggleActionButton', () => {
  it('When disabled, button is unclickable & color changed', () => {
    let value = 0;

    // test if value adds up to 1 when clicked
   const wrapperEnabled = shallow(<ToggleActionButton onClick={() => value++}>Testje1</ToggleActionButton>)
   wrapperEnabled.simulate('click');
   expect(value).toEqual(1);

   // test if value stays 1 when button is disabled and clicked
   const wrapperDisabled = shallow(<ToggleActionButton disabled onClick={() => value++}>Testje2</ToggleActionButton>)
   wrapperDisabled.simulate('click');
   expect(value).toEqual(1);

   // test if style is correct when disabled
   const wrapper = mount(
    <ThemeProvider theme={Theme}>
      <ToggleActionButton disabled onClick={() => value++}>Testje2</ToggleActionButton>
    </ThemeProvider>
   )
   expect(wrapper.childAt(0)).toHaveStyleRule('background-color', Theme().colors.disabled);
  });

  it('can pass extra props', () => {
   const wrapper = shallow(<ToggleActionButton otherProps={{id: '31'}} onClick={() => console.log('testen')}>Testje</ToggleActionButton>)
   expect(wrapper.props().id).toEqual('31');
  })

  it('style changes when level changes', () => {

    const wrapper = mount(
      <ThemeProvider theme={Theme}>
        <ToggleActionButton onClick={() => value++}>Testje2</ToggleActionButton>
      </ThemeProvider>
     );

     // test no level
     expect(wrapper.childAt(0)).toHaveStyleRule('background-color', Theme().colors.interactive)
     
     // test primary level
     wrapper.setProps({ children: <ToggleActionButton level='active' onClick={() => value++}>Testje2</ToggleActionButton>});
     expect(wrapper.childAt(0)).toHaveStyleRule('background-color', Theme().colors.toggle);

     // test secondary level
     wrapper.setProps({ children: <ToggleActionButton level='non-active' onClick={() => value++}>Testje2</ToggleActionButton>});
     expect(wrapper.childAt(0)).toHaveStyleRule('background-color', Theme().colors.interactive);

     // test random level
     wrapper.setProps({ children: <ToggleActionButton level='randomword' onClick={() => value++}>Testje2</ToggleActionButton>});
     expect(wrapper.childAt(0)).toHaveStyleRule('background-color', Theme().colors.interactive);
  })

  it('fullwidth prop takes 100% of width', () => {
    const wrapper = mount(<ToggleActionButton fullwidth onClick={() => console.log('testen')}>Testje</ToggleActionButton>)
    expect(wrapper).toHaveStyleRule('width', '100%');
  });
  
  it('icon display', () => {
    // test with image string
    const wrapper = mount(<ToggleActionButton icon='data:image/png;base64,' onClick={() => console.log('testen')}>Testje</ToggleActionButton>)
    expect(wrapper.exists('img')).toEqual(true);
    
    //empty icon tag
    wrapper.setProps({icon: ''});
    expect(wrapper.exists('img')).toEqual(false);
  });
});