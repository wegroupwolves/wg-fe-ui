...
describe('TextInput', () => {
 ...
 it('returns touched value when input is touched', () => {
  ...
  const wrapper = mount(<TextInput type="Text" name="test" setFieldTouched={(name, value) => {ctrlTouched = value; ctrlName = name}}>Test</TextInput>)
...
it('has different border color on validation', () => {
  const wrapper = mount(
    <ThemeProvider theme={Theme}>
      <TextInput name='test' type="text">Test</TextInput>
    </ThemeProvider>
  );
  
  // check if borders have default color when not touched yet
  expect(wrapper.find('input')).toHaveStyleRule('border-color', Theme().colors['outline']);
  
  // Check if border is red when errors and touched
  wrapper.setProps({ children : <TextInput errors={{ test: 'incorrect' }} touched={{ test: true }} name='test' type="text">Test</TextInput> });
  expect(wrapper.find('input')).toHaveStyleRule('border-color', Theme().colors['bad']);

  // Check if border is green when no errors but touched
  wrapper.setProps({ children : <TextInput touched={{ test: true }} name='test' type="text">Test</TextInput> });
  expect(wrapper.find('input')).toHaveStyleRule('border-color', Theme().colors['good']);
  ...
  it('Good or bad icons show at right time', () => {
    ...
    wrapper.setProps({ errors: {test: 'incorrect'}, touched: {test: true} });
