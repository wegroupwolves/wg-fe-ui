import { ToggleActionButton } from '.';
...
describe('ToggleActionButton', () => {
  it('When disabled, button is unclickable & color changed', () => {
    ...
    const wrapperEnabled = shallow(
      <ToggleActionButton onClick={() => value++}>Testje1</ToggleActionButton>,
    );
    ...
    const wrapperDisabled = shallow(
      <ToggleActionButton disabled onClick={() => value++}>
        Testje2
      </ToggleActionButton>,
    );
  ...
  it('style changes when level changes', () => {
    const wrapper = mount(
      <ThemeProvider theme={Theme}>
        <ToggleActionButton onClick={() => console.log('test')}>
          Testje2
        </ToggleActionButton>
      </ThemeProvider>,
    );
    ...
    wrapper.setProps({
      children: (
        <ToggleActionButton level="active" onClick={() => console.log('test')}>
          Testje2
        </ToggleActionButton>
      ),
    });
  ...
  it('fullwidth prop takes 100% of width', () => {
    const wrapper = mount(
      <ToggleActionButton fullwidth onClick={() => console.log('testen')}>
        Testje
      </ToggleActionButton>,
    );
